...
export const getTransferTime = (l1, l2) => {
    if (l1 === 0 && l2 === 1) return 120
    if (l1 === 1 && l2 === 0) return 120

    if (l1 === 0 && l2 === 3) return 210
    if (l1 === 3 && l2 === 0) return 210

    if (l1 === 1 && l2 === 3) return 60
    if (l1 === 3 && l2 === 1) return 60
    return 90
}

export const getUserErrors = (res) => {
    if (!res || typeof res !== 'object') return []
    return Object.values(res)
        .filter(one => one instanceof UserError || (one && typeof one === 'object' && Array.isArray(one.userErrors)))
        .map(one => one.userErrors ? one.userErrors : one)
        .flat()
}
...
//         if (d){
//             if (moment(d,'DD.MM.YYYY HH:mm:ss').isValid()){
